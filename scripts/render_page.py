...
配合 extract_outline.py:先抽图注定位关键图在第几页,再用本脚本把那几页导出成图片,
然后你(或带视觉的模型)直接看。

用法:
    python render_page.py paper.pdf 5              # 渲染第 5 页
    python render_page.py paper.pdf 5,7,8          # 渲染多页
    python render_page.py paper.pdf 5 --dpi 200    # 调清晰度(默认 150)
    python render_page.py paper.pdf 5 --out figs   # 指定输出目录(默认 ./figures)

依赖:PyMuPDF -> pip install pymupdf
输出:figures/<pdf名>_p<页码>.png,并打印每页的文字量用于判断是否扫描件。
"""
import sys
import os
import argparse
